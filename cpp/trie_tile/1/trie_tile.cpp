    ...
    if (af == AF_V4) {
        for (plen = 0; plen < PREFIX_LEN_V4_MAX; plen++) {
            for (blen = plen; (blen >= 0) && (blen > (plen-4)); blen--) {
                ...
                TRIE_TILE_EVLOG(": v4 plen/blen(" << idx << ")" << ": " << plen << "/" << blen  
                                << ": " << static_map_v4[plen][idx]); 
            }
        }
    } 
    else if (af == AF_V6) {
        for (plen = 0; plen < PREFIX_LEN_V6_MAX; plen++) {
            for (blen = plen; (blen >= 0) && (blen > (plen-4)); blen--) {
                ...
                TRIE_TILE_EVLOG(": v6 plen/blen(" << idx << ")" << ": " << plen << "/" << blen  
                                << ": " << static_map_v6[plen][idx]); 
            }
        }
                        ...
                        int base_len, int count)
{
    ...
    TRIE_TILE_DLOG(" prefix_len: " << prefix_len 
            << " base_len(" << idx << ")" << ": " << base_len << " computed_wt: " << computed_wt);

    return computed_wt;
}

void update_weight(inet_addr_family_t af, int base_len, int weight)
{
...
void add_route(inet_addr_family_t af, int prefix_len, int count) 
{
    ...
    for (base_len = prefix_len; 
        ...
        if (iterl != myheapl.end()) {
            ...
            update_weight(af, base_len, weight);
...
void test_init_entries()
{
    ...
    while(std::getline(testfile, line)) {
        ...
        if (!(iss >> af >> prefix_len >> count)) {
            TRIE_TILE_EVLOG("Error");
            break;
        }
        TRIE_TILE_EVLOG("Adding route prefix_len: " << prefix_len << " count: " << count);
        if (af.compare("v4") == 0) {
            addr_family = AF_V4;
        } else {
            addr_family = AF_V6;
...
void add_route(inet_addr_family_t af, int prefix_len, int count) 
{
    ...
    if (iterl != myheapl.end()) {
        ...
        for (base_len = prefix_len; 
            ...
            weight = get_computed_weight(af, prefix_len, base_len, count);
            entry.prefix_len   = base_len; // Add an entry per base_len
            entry.weight       = weight;
