...
ushort get_block_size(ushort number, ushort *bits)
{
  ...
  *bits = 15;
  while (mask) {
   if (mask <= number) {
       mprintf("number %d mask %d bits %d ", number, mask, *bits);
       return mask;
   }
   mask = mask >> 1;
   (*bits)--;
               ...
               ushort *bstart, ushort *bend)
{
  ...
  while ((total < count) && (block > 0)) {
    mprintf("...total %d count %d block %d", total, count, block);
    mprintf("...block_start %d start %d", block_start, start);
...
int main(int argc, char **argv)
{
  ...
  dpe = atoi(argv[4]);
  count = atoi(argv[5]);
  mprintf("Arguments SPS: %d SPE: %d DPS: %d DPE: %d count: %d", sps, spe, dps, dpe, count); 
