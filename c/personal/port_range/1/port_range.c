...
itr_check_if_ranges_intersect_get_new_range(cfg_t *cfg, cfg_t *cfg_new)
{
    ...
    if (delta > 0) { 
        ...
        np_sqrt     = np_sqrt + 1;
        leeway_1    = leeway_2 = (delta_sqrt - np_sqrt);
    }
    else {
                                        ...
                                        cfg_t *cfg_new, trace_type_t trace_type)
{
    ...
    if ((num_sports * num_dports) < NNP) {
        mprintf("Not enough ports
    }
    snp = NNP/num_sports; 
    dnp = NNP - snp;

    if ((NSPE - NSPS + 1) < np_sqrt) {
        snp = (NSPE - NSPS + 1); 
        dnp = NNP/snp - snp; 
        if ((NDPE - NDPS + 1) < dnp) {
            mprintf("*** Error - not enough ports for num paths requested *** ");
            mprintf("snp %d dnp %d NNP %d NSPS %d NSPE %d NDPS %d NDPE %d\n",
                    snp, dnp, NNP, NSPS, NSPE, NDPS, NDPE); 
            return;
        }
    }
    else if ((NSPE - NSPS + 1) < NNP/2) {
        snp = (NSPE - NSPS + 1); 
        dnp = NNP - snp; 
        if ((NDPE - NDPS + 1) < dnp) {
            mprintf("*** Error - not enough ports for num paths requested *** \n"); 
